    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.appointments.update", [$appointment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone_number') ? 'is-invalid' : '' }}" type="text" name="phone_number" id="phone_number" value="{{ old('phone_number', $appointment->phone_number) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $appointment->date) }}">
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('reserved_date') ? 'is-invalid' : '' }}" type="text" name="reserved_date" id="reserved_date" value="{{ old('reserved_date', $appointment->reserved_date) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes" id="notes">{!! old('notes', $appointment->notes) !!}</textarea>
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_city') ? 'is-invalid' : '' }}" name="user_city_id" id="user_city_id">
                    @foreach($user_cities as $id => $user_city)
                        <option value="{{ $id }}" {{ (old('user_city_id') ? old('user_city_id') : $appointment->user_city->id ?? '') == $id ? 'selected' : '' }}>{{ $user_city }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('doctor_city') ? 'is-invalid' : '' }}" name="doctor_city_id" id="doctor_city_id">
                    @foreach($doctor_cities as $id => $doctor_city)
                        <option value="{{ $id }}" {{ (old('doctor_city_id') ? old('doctor_city_id') : $appointment->doctor_city->id ?? '') == $id ? 'selected' : '' }}>{{ $doctor_city }}</option>
                    @endforeach
                </select>
