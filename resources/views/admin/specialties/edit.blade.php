    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.specialties.update", [$specialty->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $specialty->name) }}" required>
                @if($errors->has('name'))
...
<script>
    ...
    url: '{{ route('admin.specialties.storeMedia') }}',
    maxFilesize: 2, // MB
